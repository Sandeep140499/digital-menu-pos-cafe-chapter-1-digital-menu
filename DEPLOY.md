...
| **Render** | `backend` | Uses `render.yaml` or: `npm ci && npx prisma generate && npm run build` | `npm start` (migrate runs via `prestart`) | `backend/render.yaml` |
| **Railway** | `backend` | Uses `railway.toml` or: `npm ci && npx prisma generate && npm run build` | `npm start` (migrate runs via `prestart`) | `backend/railway.toml` |
