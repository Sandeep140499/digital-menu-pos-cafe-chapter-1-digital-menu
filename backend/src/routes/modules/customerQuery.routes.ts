...
customerQueryRouter.get('/', authenticate, requireRole('ADMIN'), async (req, res) => {
...
customerQueryRouter.patch('/:id/status', authenticate, requireRole('ADMIN'), async (req, res) => {
...
customerQueryRouter.post('/:id/resolve', authenticate, requireRole('ADMIN'), async (req, res) => {
  const id = Number(req.params.id);
  ...
  const orderIdStr = query.orderId
    ? `ORD${String(query.orderId).padStart(4, '0')}`
    : 'your request';
