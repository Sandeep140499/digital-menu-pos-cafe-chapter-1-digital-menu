...
function parseDirectorsEmail(s: string | null): string[] {
  if (!s || !s.trim()) return [];
  return s.split(/[,\s]+/).map((e) => e.trim()).filter((e) => e.length > 0);
}
...
directorRouter.post(
  ...
  async (req, res) => {
    ...
    return res.json({ message: "Verification email sent. Director must click the link to be added." });
  },
);
...
directorRouter.post(
  ...
  async (req, res) => {
    ...
    const verified = parseDirectorsEmail(branch.directorsEmail);
    if (!verified.includes(email.toLowerCase())) {
      return res.status(400).json({ message: "This email is not a verified director for this branch." });
    }
    ...
    return res.json({ message: "Removal email sent. Director must click the link to confirm removal." });
  },
);
