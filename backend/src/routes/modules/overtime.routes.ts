...
type ShiftWithEmployee = EmployeeShift & {
  employee: Pick<Employee, 'id' | 'name' | 'role' | 'workingHoursPerDay'>;
};
...
overtimeRouter.get('/', authenticate, requireRole('ADMIN'), async (req, res) => {
  try {
...
overtimeRouter.get('/summary', authenticate, requireRole('ADMIN'), async (_req, res) => {
  try {
    const [pendingCount, activeShifts] = await Promise.all([
      ...
      prisma.employeeShift.findMany({
        where: { shiftEnd: null },
        include: {
          employee: { select: { id: true, name: true, role: true, workingHoursPerDay: true } },
        },
      }),
    ]);
...
overtimeRouter.patch('/:id/status', authenticate, requireRole('ADMIN'), async (req, res) => {
...
overtimeRouter.get('/my-summary', authenticate, requireRole('EMPLOYEE'), async (req, res) => {
  try {
