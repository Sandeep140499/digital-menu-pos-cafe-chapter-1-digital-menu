...
performanceRouter.get('/summary', authenticate, async (req, res) => {
...
performanceRouter.get('/system', authenticate, async (req, res) => {
  ...
  const diskUsagePct = diskTotalBytes > 0 ? (diskUsedBytes / diskTotalBytes) * 100 : 0;

  // Network egress: last 24h based on perfStore bytesSent
  const snap24h = perfStore.snapshot(24 * 60 * 60 * 1000);
  const networkEgressBytes = snap24h.events.reduce((s, e) => s + ((e as any).bytesSent || 0), 0);
...
performanceRouter.get('/events', authenticate, async (req, res) => {
  const role = (req as any).user?.role;
  if (role !== 'ADMIN') return res.status(403).json({ message: 'Forbidden' });
  const windowMinutes = Math.min(Math.max(Number(req.query.windowMinutes) || 15, 1), 24 * 60);
