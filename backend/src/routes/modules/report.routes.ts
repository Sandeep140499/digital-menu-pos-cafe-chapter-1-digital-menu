...
reportRouter.post('/send-email', authenticate, requireRole('ADMIN'), async (req, res) => {
...
reportRouter.post('/salary-slips', authenticate, requireRole('ADMIN'), async (req, res) => {
...
reportRouter.get('/salary-slips', authenticate, requireRole('ADMIN'), async (req, res) => {
...
reportRouter.get(
  ...
  async (_req, res) => {
    ...
    const hasSnapshotForCurrentMonth = rows.some(r => r.yearMonth === bounds.yearMonth);

    let currentMonthLive: Record<string, unknown> | null = null;
    if (!hasSnapshotForCurrentMonth) {
      const m = await computeMonthlyMetrics(bounds.from, bounds.to, bounds.daysInMonth);
      const avgOrderValue = m.paidOrdersCount > 0 ? m.totalSales / m.paidOrdersCount : 0;
      const avgOrdersPerDayLive = avgOrdersPerDayForIncompleteMonth(m.totalOrders, new Date());
      currentMonthLive = {
        isLive: true,
        year: bounds.year,
        month: bounds.month,
    ...
    const snapshots = rows.map(r => {
      ...
      const avgOrderValue = paid > 0 ? r.totalSales / paid : 0;
...
reportRouter.post('/monthly-target', authenticate, requireRole('ADMIN'), async (req, res) => {
...
reportRouter.get('/daily-sales', authenticate, requireRole('ADMIN'), async (_req, res) => {
...
reportRouter.get('/menu-today', authenticate, requireRole('ADMIN'), async (_req, res) => {
  const { start, end } = getBusinessDayRange();
  ...
  const categories = Object.entries(byCategory).map(([category, value]) => ({
    ...
    items: Object.entries(value.items).map(([name, stats]) => ({
      name,
...
reportRouter.get('/employee-sales', authenticate, requireRole('ADMIN'), async (_req, res) => {
...
reportRouter.get('/employee-hours', authenticate, requireRole('ADMIN'), async (_req, res) => {
  const result = await prisma.employeeShift.groupBy({
    by: ['employeeId'],
    _sum: {
      totalHours: true,
...
reportRouter.get('/dashboard-summary', authenticate, requireRole('ADMIN'), async (_req, res) => {
  const { start, end } = getBusinessDayRange();
...
reportRouter.get(
  ...
  async (req, res) => {
    ...
    const byEmp = new Map<
      number,
      { count: number; sumMins: number; minMins: number; maxMins: number }
    >();
    for (const o of orders) {
      ...
      const mins = ms / 60000;
      const cur = byEmp.get(o.employeeId) ?? {
        count: 0,
        sumMins: 0,
        minMins: Number.POSITIVE_INFINITY,
        maxMins: 0,
      };
      cur.count += 1;
      cur.sumMins += mins;
