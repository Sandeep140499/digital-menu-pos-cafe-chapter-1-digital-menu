...
shiftRouter.get('/my-history', authenticate, requireRole('EMPLOYEE'), async (req, res) => {
  const employeeId = req.user!.id;
...
shiftRouter.post('/pause', authenticate, requireRole('EMPLOYEE'), async (req, res) => {
  const employeeId = req.user!.id;
...
shiftRouter.post('/resume', authenticate, requireRole('EMPLOYEE'), async (req, res) => {
  const employeeId = req.user!.id;
  const shift = await prisma.employeeShift.findFirst({
    where: { employeeId, shiftEnd: null },
  });
...
shiftRouter.post('/start', authenticate, requireRole('EMPLOYEE'), async (req, res) => {
  ...
  try {
    ...
    if (employee?.shiftStartTime) {
      const dateStr = shiftStartTime.toLocaleDateString('en-CA', {
      ...
      if (shiftStartTime.getTime() > scheduledStart.getTime()) {
        const lateDurationMinutes = Math.floor(
          (shiftStartTime.getTime() - scheduledStart.getTime()) / 60000
        );
...
// Employee: end shift
shiftRouter.post('/end', authenticate, requireRole('EMPLOYEE'), async (req, res) => {
  const employeeId = req.user!.id;
...
shiftRouter.get('/active', authenticate, requireRole('ADMIN'), async (_req, res) => {
  ...
  for (const s of activeShifts) {
    ...
    if (late) {
      ...
      lateInfo = {
        scheduledStart: day,
        actualLogin: late.actualLoginTime,
...
shiftRouter.get('/history', authenticate, requireRole('ADMIN'), async (req, res) => {
